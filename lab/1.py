...
if text:
    ...
    if total_count == 0:
        print("Ошибка: В файле нет ASCII символов.")
        exit()
    ...
    with open(CODEWORDSS_FILE, 'w', encoding='utf-8') as f:
        f.write("=== КАК ИМЕННО СТРОЯТСЯ КОДЫ (ОБЪЯСНЕНИЕ) ===\n\n")
        f.write("1. РАСЧЕТ ВЕРОЯТНОСТИ (PROBABILITY):\n")
        f.write(f"Формула: P = (Кол-во повторений символа) / (Общее кол-во символов: {total_count})\n")
    ...
    with open(KRAFT_FILE, 'w', encoding='utf-8') as f_kraft:
        f_kraft.write("ПОШАГОВЫЙ РАСЧЕТ НЕРАВЕНСТВА КРАФТА-МАКМИЛЛАНА\n\n")
        ...
        f_kraft.write("-" * 65 + "\n")
        f_kraft.write(f"ИТОГ: {km_sum:.10f} (Должно быть <= 1.0)\n")
